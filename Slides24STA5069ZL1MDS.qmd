...
- We begin with the dissimilarities $\Delta$, and do not have access to the original data matrix $X$.
  ...
  - It essentially calculates the answer in this direction:
    - dissimilarities -> centred inner products -> low-rank configuration by eigen decomposition.
  ...
  - If the dissimilarities are Euclidean ($\delta_{ij} = |\mathbf{x}_i - \mathbf{x}_j| = \sqrt{\sum (x_{ik} - x_{jk})^2}$), then the solution matches the principal components applied to the original matrix $X$ – without knowing $X$!
...
$$
\mathcal{L}_f(\mathbf{Y}_1, \mathbf{Y}_2, \ldots, \mathbf{Y}_n; \mathbf{W}) = \sum_{i<j}^{n} w_{ij}(d_{ij} - f(\delta_{ij}))^2.
$$

with respect to the $n$ $t$-dimensional points $\mathbf{y}_1, . . . ,\mathbf{y}_n  \in \mathbb{R}^t$, where $d_{ij} = \|\mathbf{y}_i - \mathbf{y}_j\|$ for $\|\cdot \|$ the Euclidean norm.

- The square root of $\mathcal{L}_f$ may be referred to as the metric stress function, when $f$ does not merely preserve ranks.
...
2. Repeat the following until convergence:
  ...
  - Move points in the direction that minimises stress.
...
p <- p +
  ...
  labs(
    x = "First dimension",
    y = "Second dimesnio",
    title = "Metric MDS representation of kinship"
  )
```
...
\begin{figure}[H]
\center
\includegraphics[width=0.6\textwidth]{_data_raw/img/morse_code_mds.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{_tmp/img/monotonic_example_1.png}
\end{figure}
...
- For each rank, the fitted value is denoted $\hat{d}_{i_kj_k}$ and termed the *disparities*:
...
p_3 <- p +
  ...
  geom_point(
    ...
    shape = 24, size = 3, col = "black", stroke = 1
  ) +
  scale_fill_manual(
    values = c("Monotonic" = "green", "Jagged" = "red", "Almost monotonic" = "orange"),
  )
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{_data_raw/img/isotonic_reg_example.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{_data_raw/img/linear_spline_example.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{_data_raw/img/alg-non_metric_mds.png}
\end{figure}
